...
class MazeKnowledgeBase:
    ...
    def ask(self, query: "MazeClause") -> bool:
        ...
        negated_props = [(prop, not query.get_prop(prop)) for prop in query.props]
        negated_clause = MazeClause(negated_props)

        new_kb = set(self.clauses)
        new_kb.add(negated_clause)
        ...
        while True:
            ...
            for i in range(len(clauses)):
                for j in range(i + 1, len(clauses)):
                    resolvents = MazeClause.resolve(clauses[i], clauses[j])

                    if any(res.is_empty() for res in resolvents):
                        return True

                    new_resolvents.update(resolvents)
            if not new_resolvents - new_kb:
                return False
            new_kb.update(new_resolvents)
    ...
    @staticmethod
    def simplify_from_known_locs(clauses: set["MazeClause"], known_pits: set[tuple[int, int]],
                                 known_safe: set[tuple[int, int]]) -> set["MazeClause"]:
        """
