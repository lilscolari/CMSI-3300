...
class MazeClause:
    ...
    def __init__(self, props: Sequence[tuple]):
        ...
        Parameters:
            props (Sequence[tuple]):
                ...
                (("P", (1, 1)), True)
        """
        self.props: dict[tuple[str, tuple[int, int]], bool | None] = dict()
        self.valid: bool = False

        for prop, truth_value in props:
            if prop in self.props:
                if self.get_prop(prop) != truth_value:
                    self.valid = True
                    self.props[prop] = None
            else:
                self.props[prop] = truth_value
        
        if None in self.props.values():
            self.valid = True
    ...
    def resolve(c1: "MazeClause", c2: "MazeClause") -> set["MazeClause"]:
        ...
        for prop, truth_value in c1.props.items():
            if prop in c2.props and c1.get_prop(prop) != c2.get_prop(prop):
                resolved_pair += 1
            else:
                new_clause[prop] = truth_value
        
        for prop, truth_value in c2.props.items():
            if prop not in c1.props:
                new_clause[prop] = truth_value

        if resolved_pair != 1:
            return set()

        new_maze_clause = MazeClause(list(new_clause.items()))
        return set() if new_maze_clause.is_valid() else {new_maze_clause}
